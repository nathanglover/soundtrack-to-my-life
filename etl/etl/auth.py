...
class SpotifyAPIAuth(requests.auth.AuthBase):
    ...
    @staticmethod
    def _get_client_secret_auth() -> (str, str):
    ...
    def _save_new_tokens(self, tokens) -> dict:
    ...
    def refresh_tokens(self):
        ...
        res = requests.post(
            self.token_url, data=data, auth=self._get_client_secret_auth()
        )
        new_tokens = res.json()
        if "refresh_token" not in new_tokens:
            new_tokens["refresh_token"] = refresh_token

        self._save_new_tokens(new_tokens)
